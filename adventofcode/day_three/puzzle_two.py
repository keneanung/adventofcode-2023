...
def __find_gear_ratios(schematic: Schematic) -> Iterable[int]:
  gears_ratios = []
  all_number_coordinates = schematic.numbers.keys()
  for symbol in schematic.symbols:
    if symbol.symbol == GEAR_CANDIDATE:
      adjacent_numbers = __find_adjacent_numbers(schematic, all_number_coordinates, symbol)
      if __adjacent_numbers_qualify_symbol_as_gear(adjacent_numbers):
        adjacent_numbers_list = __extract_number_values(adjacent_numbers)
        gears_ratios.append(__calculate_gear_ratio(adjacent_numbers_list))

  return gears_ratios

def __extract_number_values(adjacent_numbers: Iterable[Number]) -> tuple[int]:
    return tuple(adjacent_number.value for adjacent_number in adjacent_numbers)

def __adjacent_numbers_qualify_symbol_as_gear(adjacent_numbers: Sized) -> bool:
    return len(adjacent_numbers) == 2

def __find_adjacent_numbers(schematic: Schematic, all_number_coordinates: set[tuple[int, int]], symbol: Symbol) -> set[Number]:
